...
async function main() {
  ...
  if (!deleteManyResult.acknowledged) throw new Error('삭제 오류');
  ...
  const insertMemberResult = await member.insertOne({ id: 'ted', name: '방성민', isMarried: false, age: 37 });

  if (!insertMemberResult.acknowledged) throw new Error('삽입 실패');

  const updateOneResult = await member.updateOne({id: 'ted' }, { $set: { isMarried: true} }, );

  if (!updateOneResult.acknowledged) throw new Error('실패');
  ...
  // 전체 데이터 출력 코드
//   const findCursor = member.find({ $and: [{ age: { $gte: 38 } }, { isMarried: true }],
//   });
...
//   console.log(data);
const updateManyResult = await member.updateMany({}, { $set: {
